...
This file provides guidance to Claude Code when working with the `@claude-code-desktop/core` package.
...
The core package provides shared TypeScript types and utilities used by the Electron app and shared packages. It serves as the type definition layer for the Claude Code Desktop monorepo.
...
| `AgentEvent` | Events from ClaudeCodeAgent during chat (incl. task_backgrounded, shell_backgrounded, task_progress) |
...
import type {
  ...
  Message,
  AgentEvent,
} from '@claude-code-desktop/core';

// Import utilities
import { generateMessageId, debug } from '@claude-code-desktop/core';

// Or import from specific subpaths
import type { Session } from '@claude-code-desktop/core/types';
```
...
**Note:** Each MCP server has its own OAuth via `workspace_oauth::{workspaceId}`. OAuth tokens are managed separately from the main application authentication.
...
2. **Phase 2:** Move storage logic to `@claude-code-desktop/core`
