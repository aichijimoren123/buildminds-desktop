 ...
 * Uses custom API base URL if configured.
 */
async function getAnthropicClient(): Promise<Anthropic | null> {
  ...
  if (customBaseUrl) {
    ...
    debug('[summarize] Using custom API base URL:', customBaseUrl);
  }

  // Option 1: Direct API key from env (set by setAuthEnvironment)
  const envApiKey = process.env.ANTHROPIC_API_KEY;
  if (envApiKey) {
    clientOptions.apiKey = envApiKey;
  ...
  if (oauthToken) {
    clientOptions.apiKey = oauthToken;
    anthropicClient = new Anthropic(clientOptions);
