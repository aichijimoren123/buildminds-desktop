...
- Workspace: `~/.claude-code-desktop/workspaces/{id}/permissions.json`
- Source: `~/.claude-code-desktop/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.claude-code-desktop/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.claude-code-desktop/theme.json`
- Workspace: `~/.claude-code-desktop/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.claude-code-desktop/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.

### Configuration (`src/config/storage.ts`)
Multi-workspace configuration stored in `~/.claude-code-desktop/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.claude-code-desktop/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.
