 ...
 * Deep Link Handler
 *
 * Parses app:// URLs and routes to appropriate actions.
 * The actual URL scheme is configured in branding.ts (default: claudecode://)
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   {scheme}://allChats[/chat/{sessionId}]            - Chat list (all chats)
 *   {scheme}://flagged[/chat/{sessionId}]             - Chat list (flagged filter)
 *   {scheme}://state/{stateId}[/chat/{sessionId}]     - Chat list (state filter)
 *   {scheme}://sources[/source/{sourceSlug}]          - Sources list
 *   {scheme}://settings[/{subpage}]                   - Settings
 *
 * Action format:
 *   {scheme}://action/{actionName}[/{id}][?params]
 *   {scheme}://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 *   new-chat                  - Create new chat, optional ?input=text&name=name&send=true
 *   resume-sdk-session/{id}   - Resume SDK session by ID
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // Support configured scheme
    const expectedProtocol = `${DEEPLINK_SCHEME}:`
    if (parsed.protocol !== expectedProtocol) {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., claudecode://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., claudecode://allChats/chat/abc → hostname='allChats', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // claudecode://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // claudecode://allChats/..., claudecode://settings/..., etc. (compound routes)
    ...
    // claudecode://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // claudecode://action/... (no workspace - uses active window)
    if (host === 'action') {
