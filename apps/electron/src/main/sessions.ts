...
import { ClaudeCodeAgent, type AgentEvent, setPermissionMode, type PermissionMode, unregisterSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest } from '@claude-code-desktop/shared/agent'
...
import { loadWorkspaceSources, getSourcesBySlugs, type LoadedSource, type McpServerConfig, getSourcesNeedingAuth, getSourceCredentialManager, getSourceServerBuilder, type SourceWithCredential, isApiOAuthProvider, SERVER_BUILD_ERRORS } from '@claude-code-desktop/shared/sources'
import { ConfigWatcher, type ConfigWatcherCallbacks } from '@claude-code-desktop/shared/config'
import { getAuthState } from '@claude-code-desktop/shared/auth'
import { setAnthropicOptionsEnv, setPathToClaudeCodeExecutable, setInterceptorPath, setExecutable } from '@claude-code-desktop/shared/agent'
...
import { generateSessionTitle, regenerateSessionTitle, formatPathsToRelative, formatToolInputPaths, perf } from '@claude-code-desktop/shared/utils'
import { DEFAULT_MODEL } from '@claude-code-desktop/shared/config'
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL } from '@claude-code-desktop/shared/agent/thinking-levels'
...
interface ManagedSession {
  id: string
  workspace: Workspace
  agent: ClaudeCodeAgent | null  // Lazy-loaded - null until first message
  messages: Message[]
...
export class SessionManager {
  ...
  async reinitializeAuth(): Promise<void> {
    try {
      ...
      if (apiBaseUrl) {
        ...
        sessionLog.info('Set custom API Base URL:', apiBaseUrl)
      } else {
   ...
   */
  private async getOrCreateAgent(managed: ManagedSession): Promise<ClaudeCodeAgent> {
    if (!managed.agent) {
      const end = perf.start('agent.create', { sessionId: managed.id })
      const config = loadStoredConfig()
      managed.agent = new ClaudeCodeAgent({
      ...
      // The legacy onCredentialRequest callback has been removed from ClaudeCodeAgent
  ...
  private processEvent(managed: ManagedSession, event: AgentEvent): void {
    ...
    switch (event.type) {
      ...
      case 'tool_start': {
        ...
        // Use parentToolUseId from the event - ClaudeCodeAgent computes this correctly
        ...
        if (isParentTool) {
          ...
          // ClaudeCodeAgent provided the correct parent from SDK - use it
      ...
      case 'complete':
        // Complete event from ClaudeCodeAgent - accumulate usage from this turn
